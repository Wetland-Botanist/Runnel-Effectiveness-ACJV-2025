...
# Script: Script 8 - NMDS Ordination - Simplified
...
veg <- read.csv("Formatted Datasets\\Reduced Data Summary of Vegetation Plot Data.csv") %>%
  ...
  dplyr::select(-X, -richness_mean, -algae_cover_mean, -Canopy_Height_cm_mean) %>%
...
write.csv(veg_compiled, 
          "Formatted Datasets\\NMDS Runnel Vegetation Dataset - Simple.csv")
...
veg_dataset <- veg_compiled %>%
  dplyr::select(SPALT:salicornia, - live_cover) 
...
ggsave(nmds_plot_basic, 
       ...
       filename = "Output Figures\\Overall Veg NMDS - Marsh Condition - Axis 1 & 2 - Salicornia.jpg")
...
nmds_plot_species = ggplot(veg_nmds_points, 
  ...
  geom_label(data = nmds.species,
             ...
             colour = "black", 
             fontface = "bold", 
             label = row.names(nmds.species), size = 7) + 
  scale_fill_manual(values = c("orange", "darkgreen", "cornflowerblue")) + 
  # scale_colour_manual(values = c("orange", "darkgreen", "cornflowerblue")) + 
  labs(x = "Axis 1", y = "Axis 2") + 
  theme(
...
ggsave(nmds_plot_species, 
       ...
       dpi = 300,
       limitsize = FALSE, units = "in",
       filename = "Output Figures\\Runnel Veg NMDS - Treatment & Species - Axis 1 & 2 - salicornia.jpg")
...
ggsave(nmds_plot_envfit, 
       ...
       dpi = 300,
       limitsize = FALSE, units = "in",
       filename = "Output Figures\\Overall Veg NMDS - Site Characteristics - Axis 1 & 2 - salicornia.jpg")
