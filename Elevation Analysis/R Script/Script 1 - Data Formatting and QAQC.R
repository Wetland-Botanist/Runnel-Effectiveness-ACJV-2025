...
write.csv(elevs_initial,
          paste("Formatted Datasets\\", Site_Name, " Raw Elevations.csv", sep = ""))

#------------------------------------------
#Chapter 3: QAQC Elevation Dataset
#------------------------------------------

#Task 1: Format the dataset, calculate the differences in transect end and start points between Year 1 and Current Year
...
elevs_qaqc <- elevs %>%
  ...
  mutate(
    ...
    year1_transect_start = min(Distance_m[Year == year1]),
    year1_transect_end = max(Distance_m[Year == year1]),
    
    year1_transect_start_elev = elevation[Year == year1 & Distance_m == year1_transect_start],
    year1_transect_end_elev = elevation[Year == year1 & Distance_m == year1_transect_end]) %>%
  ungroup() %>%
  ...
  mutate(
    ...
    transect_start_elev = ifelse(is.na(transect_start_elev) == TRUE,
      ...
      transect_end_elev = ifelse(is.na(transect_end_elev) == TRUE,
                             ...
                             transect_end_elev)) %>%
  ungroup() %>%
#Step 4: Calculate the difference in Year 1 and Current Year Transect Elevations, Average the differences
    mutate(
...
write.csv(elevs_corrected,
          paste("Formatted Datasets\\", Site_Name, " Corrected Elevations.csv", sep = ""))
...
write.csv(elevs_qaqc_transect, 
          paste("Output Stats\\", Site_Name, " Transect QAQC Report.csv", sep = ""))
...
write.csv(elevs_qaqc_treatment_spread,
          paste("Formatted Datasets\\", Site_Name, " Treatment Mean QAQC Report.csv", sep = ""))
